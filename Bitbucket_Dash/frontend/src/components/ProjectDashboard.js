...
// Memoized contributor row component for better performance
const ContributorRow = React.memo(({ contributor, formatDate }) => (
...
const MobileProjectCard = React.memo(({ project, contributor, formatDate }) => (
  <div className="bg-white rounded-lg shadow-md p-4 space-y-3">
    <div className="flex justify-between items-start">
      <span className="inline-flex items-center px-2 py-1 bg-gradient-to-r from-blue-100 to-indigo-100 text-blue-800 rounded-full text-xs font-medium shadow-sm truncate max-w-[180px]">
    ...
    <div className="text-xs flex items-center">
      <Activity className="w-3 h-3 mr-1 text-gray-500 flex-shrink-0" />
      <span className="text-gray-700">{formatDate(contributor.commits[0].date)}</span>
    </div>
  </div>
));
...
const ProjectDashboard = () => {
  ...
  const filteredProjects = useMemo(() => {
    return Object.values(groupedByProjects)
      .filter(project => 
      ...
      <div className="max-w-7xl mx-auto space-y-4 md:space-y-6">
        ...
        <div className="bg-white rounded-xl md:rounded-2xl shadow-md md:shadow-xl p-4 md:p-6 animate-pulse">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 md:gap-4">
            {[1, 2, 3].map(i => (
              <div key={i} className="rounded-xl p-4 bg-gray-100 h-24 md:h-32">
      ...
      <div className="max-w-7xl mx-auto space-y-4 md:space-y-6">
        ...
        <div className="bg-white rounded-xl md:rounded-2xl shadow-md md:shadow-xl p-4 md:p-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 md:gap-4">
            ...
            <div className="bg-gradient-to-r from-purple-500 to-purple-600 rounded-lg md:rounded-xl p-3 md:p-5 text-white transform transition-transform duration-200 hover:scale-[1.02] sm:col-span-2 md:col-span-1">
              <div className="flex items-center justify-between">
                <div>
        ...
        <div className="md:hidden grid grid-cols-1 gap-3">
          {filteredProjects.flatMap(project => 
            Object.values(project.contributors).map((contributor, index) => (
              <MobileProjectCard 
                key={`mobile-${project.projectName}-${contributor.name}`} 
                project={project} 
                ...
                <tbody className="divide-y divide-gray-200 bg-white">
                  {filteredProjects.map(project => 
                    Object.values(project.contributors).map((contributor, contributorIndex) => (
                      ...
                      >
                        {contributorIndex === 0 && (
                          <td className="px-3 md:px-6 py-4 pl-3 md:pl-8" rowSpan={Object.keys(project.contributors).length}>
                            <span className="inline-flex items-center px-2 md:px-4 py-1 md:py-2 bg-gradient-to-r from-blue-100 to-indigo-100 text-blue-800 rounded-full text-xs md:text-sm font-medium shadow-sm truncate max-w-full">
                        ...
                        <td className="px-3 md:px-6 py-4">
                          <ContributorRow contributor={contributor} formatDate={formatDate} />
                        </td>
                        <td className="px-3 md:px-6 py-4">
                          <div className="text-xs md:text-sm">
                            ...
                            <div className="text-gray-500 mt-1.5 flex items-center">
                              <Activity className="w-3 h-3 md:w-4 md:h-4 mr-1 flex-shrink-0" />
                              <span className="truncate">Latest commit</span>
                            </div>
                          </div>
                        </td>
                      </tr>
