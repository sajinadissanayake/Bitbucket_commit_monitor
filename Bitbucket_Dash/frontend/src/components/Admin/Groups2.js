...
const Groups2 = () => {
  ...
  const ContributionStats = ({ data, repoKey }) => {
    ...
    // Prepare data for the single bar chart with percentage labels
    const contributionData = Object.entries(data.authorStats).map(([author, stats]) => ({
      author: author.split('<')[0].trim(),
      commits: stats.totalCommits,
      percentage: parseFloat(stats.percentage),
      // Create a label that combines both values
      label: `${stats.totalCommits} (${parseFloat(stats.percentage).toFixed(1)}%)`
    }));
            ...
            >
              ...
              <Tooltip 
                formatter={(value, name, props) => {
                  if (name === 'commits') {
                    return [`${value} (${props.payload.percentage.toFixed(1)}%)`, 'Commits'];
                  }
                  return [value, name];
                }}
              />
